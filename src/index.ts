...
const WCLIENT_ID =
  process.env.WCLIENT_ID || IS_PRODUCTION ? "PROD_CLIENT" : "DEV_CLIENT";
...
import { HojeTemCommand } from "./commands/hoje-tem";
...
import { YoutubeVideoResumeCommand } from "./commands/gemini/youtube-video-resume.command";
...
const config = {
  authStrategy: new LocalAuth({
    clientId: IS_PRODUCTION ? WCLIENT_ID : undefined,
    dataPath: IS_PRODUCTION ? "/app/auth_data" : undefined,
  }),
  puppeteer: {
    args: ["--no-sandbox", "--disable-backgrounding-occluded-windows"],
  },
};
...
client.on("authenticated", async () => {
  ...
  handlers.forEach((handlerConstructor) => {
    ...
    registerCommand(
      registerNormalizedCommand(handler.command),
      handler,
      RegisteredHandlers,
    );
    for (const command of handler.alternativeCommands) {
      registerCommand(
        registerNormalizedCommand(command),
        handler,
        RegisteredHandlers,
      );
    }
  ...
  runners.forEach(async (runnerConstructor) => {
    ...
    console.info(
      `Registering runner ${runner.runnerName} to run every ${runner.runEveryNMinutes} minute(s)`,
    );
    const catchFn = (exception: Error) =>
      console.error(
        `Error while trying to run ${runner.runnerName}`,
        exception,
      );
...
const registerNormalizedCommand = (command: string) => {
  if (!IS_PRODUCTION) {
    command += "-dev";
  }
...
const handleMessage = async (msg: Message) => {
  ...
  console.info(
    `Received message from ${msg["_data"].notifyName} at ${chat.name}: `,
  );
  ...
  if (typeof msg.body === "string") {
    const [command, ...argsArray] = msg.body
      .replace(/\xA0/g, " ")
      .split(" ")
      .map((a) => a.trim())
      .filter((arg) => !!arg.trim());
    ...
    if (handler) {
      console.info(
        `Message contains a registered command ${normalizedCommand}`,
      );
          ...
          if (word.toLowerCase() === keyworkd.toLowerCase()) {
            for (const allowedGroup of reaction.allowedGroups) {
              ...
              if (isId(allowedGroup)) {
                ...
                const chatList = CHATGROUPS.find(
                  (chatGroup) => chatGroup.name === allowedGroup,
                );
                shouldReact =
                  !!chatList && chatList.ids.includes(chat.id._serialized);
              }
              if (shouldReact) {
                const randomReaction =
                  reaction.reactions[
                    randomIntFromInterval(0, reaction.reactions.length - 1)
                  ];

...
async function retrieveData<T>(dataType: DataType, ...tickers: string[]): Promise<FinancialAPIRequestResult<T>> {
    ...
    if (apiResult.code !== 200 && apiResult.code !== 113) {
    ...
    if (apiResult.response) {
        ...
        return {
            ...
            failed: searchValue.split(',').filter(s => !success.includes(s))
        };
    }
    return {
        success: [] as T[],
        failed: searchValue.split(',')
    };
}

export async function getStockInfo(tickers: string[]): Promise<FinancialAPIRequestResult<StockInfo>> {
    const successTickers: StockInfo[] = [];
    let faileds = [];
    ...
    if (tickersToRetrieve.length) {
        try {
            ...
            for (const result of results.success) {
                ...
                successTickers.push(result)
            }
            faileds = results.failed;
        } catch (e) {
            console.log(e);
        }
    }
    return {
        success: successTickers.sort((a, b) => { if (a.ticker > b.ticker) return 1; if (b.ticker > a.ticker) return -1; return 0; }),
        failed: faileds
    }
}
